...
> 创建：2026-08-29 · 当前状态：**七项决策全部拍板（001–007），待总体设计文档结构审阅**
...
- [x] 阶段0.5 核心架构决策：七项 ADR 拍板并分类落盘（001–007，目标架构冻结）
- [x] 阶段1 范围分级（ADR-007：P0=A+B+C / P1=D+F+RAG Query Harness / P2=E+G；分期只裁功能不裁架构）
- [ ] 阶段2 总体设计文档：**结构先给用户审** → 成文（架构总图、MySQL schema 含 gotd session 方案、.env v2、Qdrant 设计、部署、接口边界清单）
- [ ] 阶段3 实施计划（Go 版 P0，待总体设计批准后编写）
- [ ] 阶段4 P0 实施（**动代码前需用户确认实施计划**）
