...
Sakura-Nexus 重写的已拍板决策。每项一个文件，自包含（背景 / 决策 / 理由 / 备选与否决原因）。修改已拍板决策须修订对应文件并在状态中注明，不得静默变更。

> **项目定名（2026-08-29）**：新项目正式名称 **Sakura-Nexus**（仓库 `github.com/Sakura520222/Sakura-Nexus`，module path 与产物名随之）；文中「Sakura-Bot / v1」一律指旧项目 `E:\项目\Sakura-Bot`。
...
- 总体设计：**已于 2026-08-29 经用户批准并全部冻结**（overview + 01–07，R3.1.1）
- Go P0 实施计划：编写中（docs/plans/p0-implementation.md）——**计划获批前零实现代码**
