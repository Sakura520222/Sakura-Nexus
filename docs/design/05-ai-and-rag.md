...
| Vision | `Describe(image)` | 图片结构化描述 | P2 |
...
Telegram New/Edit/Delete
  ...
  → RAG derived-index 队列
      ├─ New：dense embedding → Qdrant upsert → embedding_state=1
      ├─ Edit：current_revision+1 → 重 embedding → 同 UUID upsert（覆盖旧版）
      └─ Delete：invalidation（不可静默丢弃）→ Qdrant point 删除
```

- P1 不做 AI 增强（ai_meta 留空）；P2 增加分类/标签/实体（写入 ai_meta，payload 同步）。
- `embedding_state`（02 §2.3）驱动 **repair 任务**（周期维护）：扫描 `state=0(pending)` 的消息补索引——兼作队满丢弃的补偿通道。
...
- 输入：检索结果（已选 top 8–15）。
- 组装：按 `published_at` 升序；每条 `{时间, 作者(user_id/username), 频道, 文本, t.me 深链}`；token 预算可配（默认 ~8k）；超预算按相关度保留头部。
...
→ 按 messages.id 升序分批（batch=50，embedding 限速可配）
→ 每批后写 reindex_state checkpoint（断点续传；限流/重启不影响）
...
- `POST /rag/answer`：query + 上一步选中 context → `AIProvider.Chat` → 返回 `AIResponse`（前端直接渲染，验证全链路：Telegram → MySQL → embedding → Qdrant → retrieval → Context Builder → LLM）。
...
- 触发时上下文 = 该帖近期完整对话（含未 @ 的消息，按时间序）+ thread memory + channel RAG（Retriever 按频道过滤）。
